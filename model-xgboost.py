...
XGB_model = xgb.XGBRegressor(objective='reg:linear', max_depth=6, learning_rate=0.045, subsample=0.8, colsample_bytree=0.9, seed=0)
model = grid_search.GridSearchCV(estimator=XGB_model, param_grid=param_grid, scoring='neg_mean_squared_error', verbose=10, n_jobs=3, iid=True, refit=True, cv=5)
...
sub_cv.to_csv('submission_xgb-89-045-dropNA.csv', index=False)
